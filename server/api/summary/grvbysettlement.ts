...
export default defineEventHandler(async (req) => {
    ...
    try {
        ...
        summary.forEach(item => {
            const { settlement } = item._id;
            if (!formattedSummary[settlement]) {
                formattedSummary[settlement] = Array(allDates.length).fill(0); // Initialize array with zeros
            }
        });

        // Populate the counts for existing data points
        summary.forEach(item => {
            const { year, month, settlement } = item._id;
            const date = `${new Intl.DateTimeFormat('en-US', { month: 'short' }).format(new Date(year, month - 1))}-${year}`;
            const index = allDates.indexOf(date);
            if (index !== -1) {
                formattedSummary[settlement][index] = item.count;
            }
        });

        // Convert formattedSummary to the structure Highcharts expects
        const series = Object.keys(formattedSummary).map(settlement => ({
            name: settlement,
            data: formattedSummary[settlement]
        }));
